...
async function scanCommunity(communityId: string, organizationId: string | null): Promise<Scan> {
  ...
  // De-dup against notifications already raised in the last 3 days.
  const existing = await prisma.notification.findMany({
    where: { communityId, createdAt: { gte: new Date(nowTs - 3 * 86_400_000) }, relatedEntityType: { in: RELATED_TYPES } },
