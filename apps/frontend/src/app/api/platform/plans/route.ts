...
export async function GET() {
  ...
  const plans = await prisma.plan.findMany({ where: { isActive: true }, include: { entitlements: true }, orderBy: { name: "asc" } });
  return NextResponse.json({ plans: plans.map((plan) => ({ ...plan, maxStorageBytes: plan.maxStorageBytes?.toString() || null })) });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ plan: { ...plan, maxStorageBytes: plan.maxStorageBytes?.toString() || null } }, { status: 201 });
  } catch {
