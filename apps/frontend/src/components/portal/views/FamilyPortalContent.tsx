...
export default function FamilyPortalContent({ tab }: FamilyPortalContentProps) {
  ...
  if (tab === "expenses") {
    const STATUS: Record<string, { label: string; badge: string; bar: string }> = {
      DRAFT: { label: "Draft", badge: "bg-gray-100 text-gray-700", bar: "bg-gray-400" },
      SENT: { label: "Sent", badge: "bg-blue-100 text-blue-700", bar: "bg-blue-500" },
      PAID: { label: "Paid", badge: "bg-green-100 text-green-700", bar: "bg-green-500" },
      OVERDUE: { label: "Overdue", badge: "bg-red-100 text-red-700", bar: "bg-red-500" },
    ...
    const invoices = invoiceRows.map((inv: Record<string, unknown>, i: number) => {
      const total = num(inv.totalAmount);
      const paid = num(inv.amountPaid);
      const balance = Math.max(0, total - paid);
      const status = String(inv.status ?? "DRAFT");
      const dueTs = inv.dueDate ? new Date(String(inv.dueDate)).getTime() : 0;
      const overdue = (status === "OVERDUE") || (balance > 0 && dueTs > 0 && nowTs > 0 && dueTs < nowTs && status !== "PAID" && status !== "CANCELLED");
      return {
        id: String(inv.id ?? i),
        number: String(inv.invoiceNumber ?? "—"),
        description: String(inv.description ?? ""),
        total, paid, balance, status, dueTs, overdue,
        periodStart: inv.billingPeriodStart ? new Date(String(inv.billingPeriodStart)) : null,
        periodEnd: inv.billingPeriodEnd ? new Date(String(inv.billingPeriodEnd)) : null,
      };
    });

    const active = invoices.filter((v) => v.status !== "CANCELLED");
    const totalBilled = active.reduce((s, v) => s + v.total, 0);
    const totalPaid = active.reduce((s, v) => s + v.paid, 0);
    const balanceDue = active.reduce((s, v) => s + v.balance, 0);
    const overdueList = invoices.filter((v) => v.overdue);
    ...
    const statusCounts = Object.keys(STATUS).map((k) => ({ key: k, count: invoices.filter((v) => v.status === k).length })).filter((s) => s.count > 0);
    const maxStatus = Math.max(1, ...statusCounts.map((s) => s.count));

    const q = invSearch.trim().toLowerCase();
    const filtered = invoices
      .filter((v) => invStatus === "all" || (invStatus === "overdue" ? v.overdue : v.status === invStatus))
      .filter((v) => !q || v.number.toLowerCase().includes(q) || v.description.toLowerCase().includes(q))
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center gap-2">
              <Receipt className="w-6 h-6 text-yellow-500 flex-shrink-0" /> Billing
            </h1>
            <p className="text-gray-600 flex items-center gap-2 text-sm mt-1">
              <span className="inline-flex items-center gap-1 text-green-600"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> Live</span>
              Invoices &amp; payments for {relativeDisplayName}
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="inline-flex rounded-lg border border-gray-300 overflow-hidden bg-white">
              <button onClick={() => setBillView("list")} className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition ${billView === "list" ? "bg-yellow-400 text-black" : "text-gray-700 hover:bg-gray-50"}`}><Receipt className="w-4 h-4" /> Invoices</button>
              <button onClick={() => setBillView("analytics")} className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition border-l border-gray-300 ${billView === "analytics" ? "bg-yellow-400 text-black" : "text-gray-700 hover:bg-gray-50"}`}><Activity className="w-4 h-4" /> Analytics</button>
            </div>
            <button onClick={() => exportInvoicesCsv(invoices)} disabled={invoices.length === 0} className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition text-sm font-medium disabled:opacity-50"><FileText className="w-4 h-4" /> Export</button>
            <button onClick={() => void refetchInvoices()} className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition text-sm font-medium"><RefreshCw className="w-4 h-4" /> Refresh</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <MoneyStat label="Overdue" value={fmt(overdueAmount)} icon={AlertTriangle} tone={overdueAmount > 0 ? "red" : "green"} sub={overdueList.length ? `${overdueList.length} invoice${overdueList.length === 1 ? "" : "s"}` : undefined} />
        </div>

        {billView === "analytics" && <BillingAnalytics invoices={invoices} />}

        {billView === "list" && (<>
        {/* Payment progress + status breakdown */}
        {invoices.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-gray-900">Payment Progress</h3>
                <span className="text-sm font-bold text-gray-700">{paidPct}% paid</span>
              </div>
              <div className="w-full h-4 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-green-400 to-green-500 transition-all duration-500" style={{ width: `${paidPct}%` }} />
              </div>
              <p className="text-xs text-gray-500 mt-2">{fmt(totalPaid)} paid of {fmt(totalBilled)} billed • {fmt(balanceDue)} outstanding</p>
            </div>
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              ...
              <div className="space-y-2">
                {statusCounts.map(({ key, count }) => (
                  <div key={key} className="flex items-center gap-3">
                    <span className="w-20 text-xs font-semibold text-gray-600">{statusMeta(key).label}</span>
                    <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full ${statusMeta(key).bar} transition-all`} style={{ width: `${(count / maxStatus) * 100}%` }} />
                    </div>
                    <span className="w-6 text-sm font-bold text-gray-700 text-right">{count}</span>
                  </div>
                ))}
        ...
        {/* Filters */}
        <div className="flex flex-col sm:flex-row gap-3">
          <select value={invStatus} onChange={(e) => setInvStatus(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-yellow-400 outline-none">
          ...
          <div className="relative flex-1">
            <Search className="absolute left-3 top-2.5 w-5 h-5 text-gray-400" />
            <input type="text" placeholder="Search invoice number or description…" value={invSearch} onChange={(e) => setInvSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-yellow-400 focus:border-transparent outline-none" />
          </div>
        </div>
        ...
        {invoiceLoading && invoiceRows.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((v) => {
              ...
              return (
                <div key={v.id} className={`bg-white rounded-lg border p-4 ${v.overdue ? "border-red-200" : "border-gray-200"}`}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      <h4 className="font-bold text-gray-900 truncate">{v.number}</h4>
                      {v.description && <p className="text-xs text-gray-600 truncate">{v.description}</p>}
                    </div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 ${v.overdue ? "bg-red-100 text-red-700" : statusMeta(v.status).badge}`}>{v.overdue ? "Overdue" : statusMeta(v.status).label}</span>
                  </div>
                  {(v.periodStart && v.periodEnd) && (
                    <p className="text-xs text-gray-500 mb-2">Period: {v.periodStart.toLocaleDateString()} – {v.periodEnd.toLocaleDateString()}</p>
                  )}
                  <div className="grid grid-cols-3 gap-2 text-center my-3">
                    <div><p className="text-xs text-gray-500">Total</p><p className="font-bold text-gray-900">{fmt(v.total)}</p></div>
                    <div><p className="text-xs text-gray-500">Paid</p><p className="font-bold text-green-600">{fmt(v.paid)}</p></div>
                    <div><p className="text-xs text-gray-500">Balance</p><p className={`font-bold ${v.balance > 0 ? "text-amber-600" : "text-gray-400"}`}>{fmt(v.balance)}</p></div>
                  </div>
                  <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full ${pct === 100 ? "bg-green-500" : "bg-yellow-400"} transition-all`} style={{ width: `${pct}%` }} />
                  </div>
                  <p className={`text-xs mt-2 flex items-center gap-1 ${v.overdue ? "text-red-600 font-semibold" : "text-gray-500"}`}>
                    <Clock className="w-3.5 h-3.5" /> Due {v.dueTs ? new Date(v.dueTs).toLocaleDateString() : "—"}
                  </p>
                </div>
              );
            })}
