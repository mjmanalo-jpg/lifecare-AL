              ...
              <tbody>
                {forms.moveIn.items.map((it) => (
                  <tr key={it.id} className="border-t border-gray-100">
                    <td className="py-2 text-center align-middle"><input type="checkbox" disabled={!canEdit} checked={!!it.checked} onChange={(e) => update((f) => ({ ...f, moveIn: { ...f.moveIn, items: f.moveIn.items.map((x) => x.id === it.id ? { ...x, checked: e.target.checked } : x) } }))} className="h-4 w-4 cursor-pointer accent-[var(--clinical-panel)] disabled:cursor-default" /></td>
                    <td className="py-2 pr-2 align-middle text-gray-800">{it.label || <input disabled={!canEdit} placeholder="Item" value={s(it.label)} onChange={(e) => update((f) => ({ ...f, moveIn: { ...f.moveIn, items: f.moveIn.items.map((x) => x.id === it.id ? { ...x, label: e.target.value } : x) } }))} className={cell} />}</td>
                    <td className="py-2 pr-2 align-middle"><input disabled={!canEdit} value={s(it.quantity)} onChange={(e) => update((f) => ({ ...f, moveIn: { ...f.moveIn, items: f.moveIn.items.map((x) => x.id === it.id ? { ...x, quantity: e.target.value } : x) } }))} className={cell} /></td>
                    <td className="py-2 pr-2 align-middle"><input disabled={!canEdit} placeholder="Name & date" value={s(it.checkedBy)} onChange={(e) => update((f) => ({ ...f, moveIn: { ...f.moveIn, items: f.moveIn.items.map((x) => x.id === it.id ? { ...x, checkedBy: e.target.value } : x) } }))} className={cell} /></td>
                    {canEdit && <td className="py-2 align-middle"><button onClick={() => update((f) => ({ ...f, moveIn: { ...f.moveIn, items: f.moveIn.items.filter((x) => x.id !== it.id) } }))} aria-label="Remove item" className="text-gray-300 hover:text-red-500"><X className="h-3.5 w-3.5" /></button></td>}
                  </tr>
                ))}
              </tbody>
            </table>
