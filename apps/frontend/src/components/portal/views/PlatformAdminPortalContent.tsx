...
export default function PlatformAdminPortalContent({ tab = "dashboard" }: { tab?: string }) {
  ...
  return <div className="space-y-6"><section className="rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-700 to-indigo-700 p-5 text-white shadow-lg"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><div className="rounded-xl bg-white/15 p-2.5"><ShieldCheck className="h-7 w-7"/></div><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-100">LCMS SaaS Control Plane</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Platform Admin Portal</h1><p className="mt-2 max-w-3xl text-sm text-blue-100">Provision customers, govern subscriptions and access, monitor capacity, and review platform security without entering resident clinical workflows.</p></div></div>{loading && <Loader2 className="h-5 w-5 animate-spin text-blue-100"/>}</div></section>{error && <div className="flex flex-col gap-3 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 sm:flex-row sm:items-center sm:justify-between"><span>{error}</span><button onClick={() => void load()} className="inline-flex flex-none items-center justify-center gap-2 rounded-lg border border-rose-300 bg-white px-3 py-2 text-xs font-bold"><RefreshCw className="h-4 w-4"/>Retry</button></div>}{views[tab] || overview}</div>;
}
