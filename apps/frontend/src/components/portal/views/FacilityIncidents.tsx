              ...
              <div key={i.id} className={`rounded-lg border transition ${i.resolved ? "bg-green-50/60 border-green-200" : "bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm"}`}>
                <div className="p-3 sm:p-4 flex items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1.5 sm:gap-3 mb-1.5">
                      <div className="min-w-0">
                        <h4 className={`text-base font-bold break-words ${i.resolved ? "line-through text-gray-500" : "text-gray-900"}`}>{i.type}</h4>
                        <p className="text-sm text-gray-600 flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                          <span className="inline-flex items-center min-w-0"><User className="w-3 h-3 flex-shrink-0 mr-0.5" /><span className="truncate">{i.resident}</span></span>
                          <span className="inline-flex items-center whitespace-nowrap"><MapPin className="w-3 h-3 mr-0.5" />Room {i.room}</span>
                          <span className="inline-flex items-center whitespace-nowrap"><Clock className="w-3 h-3 mr-0.5" />{relTime(String(i.timestamp), nowTs)}</span>
                        </p>
                      </div>
