...
type Staff = { id: string; position: string; department?: string | null; isActive: boolean; isApproved: boolean; needsFirstPassword?: boolean; community?: { id: string; name: string } | null; user: { id: string; name: string; email: string; phone?: string | null; role: string; isActive: boolean } };
...
export default function OrganizationAdminPortalContent({ tab = "dashboard" }: { tab?: string }) {
  ...
  async function updateStaffAccount(event: FormEvent<HTMLFormElement>) {
  ...
  async function resetStaffPassword(staff: Staff) {
    ...
    if (!confirmed) return;
    setBusy(true); setError(""); setNotice("");
    const response = await fetch(`/api/organization-admin/staff/${staff.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ resetPassword: true }) });
    const body = await response.json().catch(() => ({})); if (response.ok) { setNotice(`${staff.user.name} will set a new password on their next sign-in.`); await load(); } else setError(body.error || "Reset failed"); setBusy(false);
  }
  async function toggleStaffActive(staff: Staff) {
    const activating = !staff.user.isActive;
    const { confirmed } = await openGlobalConfirm(activating
      ? { title: `Reactivate ${staff.user.name}?`, description: "They regain sign-in access with their company name + mobile number.", confirmText: "Activate", cancelText: "Cancel", variant: "warning" }
      : { title: `Deactivate ${staff.user.name}?`, description: "They lose sign-in access until reactivated. Their records and history are kept.", confirmText: "Deactivate", cancelText: "Cancel", variant: "danger" });
    if (!confirmed) return;
    setBusy(true); setError(""); setNotice("");
    const response = await fetch(`/api/organization-admin/staff/${staff.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: activating }) });
    const body = await response.json().catch(() => ({})); if (response.ok) { setNotice(`${staff.user.name} ${activating ? "reactivated" : "deactivated"}.`); await load(); } else setError(body.error || "Update failed"); setBusy(false);
  }
  ...
  const STAFF_ROLES: [string, string][] = [["CARE_MANAGER", "Care Manager"], ["CRM", "CRM Specialist"], ["RESIDENT_COORDINATOR", "Resident Coordinator"], ["NURSE", "Nurse"], ["CAREGIVER", "Caregiver"], ["PHYSICIAN", "Physician"], ["FACILITY_ADMIN", "Facility Admin"], ["BILLING_ADMIN", "Billing Admin"], ["NUTRITIONIST", "Nutritionist"], ["KITCHEN", "Kitchen"], ["HOUSEKEEPING", "Housekeeping"], ["MAINTENANCE", "Maintenance"], ["SECURITY", "Security"], ["FLEET_MANAGEMENT", "Fleet Manager"], ["DRIVER", "Driver"], ["SUPERADMIN", "Super Admin"]];
  ...
  // Group staff records by user so each person appears once with all their communities.
  type GroupedStaff = { userId: string; user: Staff["user"]; communities: { id: string; name: string }[]; staffRecords: Staff[]; primary: Staff };
  const groupedStaff: GroupedStaff[] = useMemo(() => {
    const map = new Map<string, GroupedStaff>();
    for (const s of allStaff) {
      let group = map.get(s.user.id);
      if (!group) {
        group = { userId: s.user.id, user: s.user, communities: [], staffRecords: [], primary: s };
        map.set(s.user.id, group);
      }
      group.staffRecords.push(s);
      if (s.community && !group.communities.some((c) => c.id === s.community!.id)) {
        group.communities.push(s.community);
      }
      // Prefer the active/approved record as primary
      if (s.isApproved && s.user.isActive) group.primary = s;
    }
  ...
  const invitations = <div className="space-y-5">{staffAccountForm}<Card title={`All staff accounts${groupedStaff.length ? ` (${groupedStaff.length})` : ""}`} subtitle="Every staff member registered for this organization. Their mobile number is their company sign-in." icon={<Users className="h-5 w-5"/>}><div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-500"><tr><th className="p-3">Person</th><th className="p-3">Role</th><th className="p-3">Mobile (sign-in)</th><th className="p-3">Community</th><th className="p-3">Position</th><th className="p-3">Status</th><th className="p-3">Actions</th></tr></thead><tbody>{groupedStaff.map((group) => <tr key={group.userId} className="border-b last:border-0"><td className="p-3"><b className="block">{group.user.name}</b>{realEmail(group.user.email) ? <span className="text-xs text-slate-500">{group.user.email}</span> : <span className="text-xs text-slate-400">No email</span>}</td><td className="p-3">{roleLabel(group.user.role)}</td><td className="p-3 text-xs font-medium tabular-nums text-slate-700">{fmtMobile(group.user.phone)}</td><td className="p-3 text-xs">{group.communities.map((cc) => cc.name).join(", ") || "—"}</td><td className="p-3 text-xs">{group.primary.position}</td><td className="p-3"><Badge value={group.primary.isApproved ? (group.user.isActive ? "ACTIVE" : "SUSPENDED") : "PENDING"}/>{group.staffRecords.some((sr) => sr.needsFirstPassword) ? <span className="mt-1 block text-[10px] font-semibold text-amber-600">First-time pending</span> : <span className="mt-1 block text-[10px] text-slate-400">Password set</span>}</td><td className="p-3"><div className="flex flex-wrap items-center gap-1.5"><button onClick={() => { setError(""); setNotice(""); setEditStaffRole(group.user.role); setEditStaff(group.primary); }} className="rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button><button disabled={busy} onClick={() => void toggleStaffActive(group.primary)} className={`rounded-lg border px-2.5 py-1 text-xs font-semibold ${group.user.isActive ? "border-amber-200 text-amber-700 hover:bg-amber-50" : "border-emerald-200 text-emerald-700 hover:bg-emerald-50"}`}>{group.user.isActive ? "Deactivate" : "Activate"}</button>{!group.staffRecords.some((sr) => sr.needsFirstPassword) && <button disabled={busy} onClick={() => void resetStaffPassword(group.primary)} title="Clear their password so they set a new one on next sign-in" className="rounded-lg border border-blue-200 px-2.5 py-1 text-xs font-semibold text-blue-700 hover:bg-blue-50">Reset password</button>}{availableCommunitiesFor(group.primary).length > 0 && <button disabled={busy} onClick={() => { setError(""); setNotice(""); setAssignCommunityId(""); setAssignStaff(group.primary); }} title="Give this person access to another community — same role, switchable" className="rounded-lg border border-indigo-200 px-2.5 py-1 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">Assign to community</button>}</div></td></tr>)}</tbody></table>{!groupedStaff.length && <p className="text-sm text-slate-500">No staff accounts yet.</p>}</div></Card>{editStaff && (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setEditStaff(null)}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={updateStaffAccount} className="grid gap-3 sm:grid-cols-2">
          <div className="sm:col-span-2"><label className={labelCls}>Full name</label><input required name="name" defaultValue={editStaff.user.name} className={fieldCls}/></div>
          <div><label className={labelCls}>Mobile number <span className="text-blue-600">*unique sign-in</span></label><input required type="tel" name="phone" defaultValue={editStaff.user.phone || ""} className={fieldCls}/></div>
          <div><label className={labelCls}>Email <span className="font-normal text-slate-400">(optional)</span></label><input type="email" name="email" defaultValue={realEmail(editStaff.user.email) || ""} placeholder="name@company.com" className={fieldCls}/></div>
          <div><label className={labelCls}>Role</label><select required name="role" value={editStaffRole} onChange={(e) => setEditStaffRole(e.target.value)} className={fieldCls}>{STAFF_ROLES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          <div><label className={labelCls}>Community</label><select required name="communityId" defaultValue={editStaff.community?.id || ""} className={fieldCls}><option value="" disabled>Select community</option>{organization?.communities.filter((item) => item.isActive).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
          <div><label className={labelCls}>Position / title</label><input required name="position" defaultValue={editStaff.position} className={fieldCls}/></div>
          <div><label className={labelCls}>Department <span className="font-normal text-slate-400">(optional)</span></label><input name="department" defaultValue={editStaff.department || ""} className={fieldCls}/></div>
          <div className="sm:col-span-2 flex gap-2 pt-1">
            <button type="button" onClick={() => setEditStaff(null)} className="flex-1 rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Cancel</button>
